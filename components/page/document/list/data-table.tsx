...
import {deleteDocumentAction, destroyDocumentAction} from "@/actions/document";
import {Button} from "@/components/ui/button";
import {FileText, Download, Plus} from "lucide-react";
import PageDocumentListFilterTable from "@/components/page/document/list/filter-table";
...
export function PageDocumentListTable({ className, ...props }: React.ComponentProps<"div">) {
	...
	const [selectedDocument, setSelectedDocument] = React.useState<DocumentResponse | null>(null);
				...
				<>
							...
							{selectedDocument && (
								<div className="grid gap-4 py-4">
									...
									<div>
										...
										<p className="font-mono text-sm">{selectedDocument.filePath}</p>
									</div>

									<div className="grid grid-cols-2 gap-4">
										<div>
											<p className="text-sm text-muted-foreground">Uploaded By</p>
											<p className="font-medium">User ID: {selectedDocument.uploadedBy}</p>
										</div>
										<div>
									...
									{selectedDocument.deletedAt && (
										<div>
											...
											<p className="font-medium text-red-500">
												{new Date(selectedDocument.deletedAt).toLocaleString("id-ID")}
											</p>
										</div>
									)}
								</div>
					...
					<AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
						<AlertDialogContent>
							<AlertDialogHeader>
								<AlertDialogTitle>
									{isHardDelete ? "Permanently Delete Document?" : "Soft Delete Document?"}
								</AlertDialogTitle>
								<AlertDialogDescription>
									{isHardDelete
									...
									{selectedDocument && (
										<span className="mt-2 font-medium">Document: {selectedDocument.originalName}</span>
									)}
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
